...
def run(dry_run: bool = False):
    ...
    if dry_run:
        ...
        print("档位变化：", [(e["ticker"], e["reason"]) for e in events] or "无")
        return new

    data["classification"] = {"updated": today, "tickers": new}
    wm.save_watchlist(data)
    if events:
        wm.log_watchlist_changes(events)
    log.info("✅ classification 已写入 watchlist.json（%d 只，档位变化 %d）", len(new), len(events))
    return new
