...
def compute() -> dict:
    ...
    # ── A) 脆弱性因子(True = 更脆) ──
    def frag(name, val_str, on, detail):
        return {"name": name, "value": val_str, "fragile": bool(on), "detail": detail}
    ...
    factors = [
        frag("VIX 绝对低位", f"{vix:.1f}" if not np.isnan(vix) else "—", f1, f"<{VIX_LOW:.0f} 偏脆"),
        frag("期限结构 VIX3M/VIX", f"{contango*100:+.0f}%" if not np.isnan(contango) else "—", f2,
             f"contango>{CONTANGO_TH*100:.0f}% 且 VIX<{VIX_CALM_CAP:.0f} 偏脆"),
        frag("实现波动分位(1y)", f"{rvol_pct*100:.0f}%ile" if not np.isnan(rvol_pct) else "—", f3,
             f"<{RVOL_PCTL*100:.0f}分位 偏脆(vol-target 加杠杆)"),
        frag("QQQ 高于 200日线", f"{stretch*100:+.0f}%" if not np.isnan(stretch) else "—", f4,
             f">{STRETCH_TH*100:.0f}% 偏脆(动量拉伸)"),
        frag("Burry 篮子 RSI 中位", f"{basket_rsi:.0f}" if not np.isnan(basket_rsi) else "—", f5,
             f">{BASKET_RSI:.0f} 偏脆(拥挤)"),
    ]
    frag_score = sum(1 for f in factors if f["fragile"])
    if frag_score >= 4:
        frag_color, frag_label = "red", "🔴 高度脆弱(干柴充足)"
    elif frag_score >= 2:
        frag_color, frag_label = "amber", "🟡 中度脆弱"
    else:
        frag_color, frag_label = "green", "🟢 低脆弱"
    ...
    # 每个 tell 给出倾向:"mech"(机械/仓位型) / "reco"(衰退/基本面型) / "neutral"
    tells = []

    def tell(name, val, lean, why):
        tells.append({"name": name, "value": val, "lean": lean, "why": why})

    # 1) VIX 期限结构
    if not np.isnan(contango):
        if contango < -0.05:   # backwardation(前月高于3月)
            tell("VIX 期限结构", f"{contango*100:+.0f}% (倒挂)", "stress", "骤然倒挂=急性恐慌,两型都有→看其余 tell 拆分")
        else:
            tell("VIX 期限结构", f"{contango*100:+.0f}%", "neutral", "尚未倒挂,恐慌未极端")

    # 2) 长债 TLT 反应
    if not np.isnan(tlt_r):
        if tlt_r >= 0.005:
            tell("长债 TLT", f"{tlt_r*100:+.1f}%", "reco", "避险买盘涌入长债=冲着经济/降息去的=衰退型")
        elif tlt_r <= -0.003:
            tell("长债 TLT", f"{tlt_r*100:+.1f}%", "mech", "股债黄金一起跌=为流动性无差别抛售=机械/仓位型")
        else:
            tell("长债 TLT", f"{tlt_r*100:+.1f}%", "neutral", "长债基本走平,无强避险")
    ...
    if not np.isnan(hyg_r) and not np.isnan(spy_r):
        ...
        if excess < -0.005:
            tell("高收益信用 HYG", f"{hyg_r*100:+.1f}% (超beta {excess*100:+.1f}%)", "reco",
                 "信用比股票 beta 跌得更狠=违约/基本面担忧=衰退型")
        else:
            tell("高收益信用 HYG", f"{hyg_r*100:+.1f}% (超beta {excess*100:+.1f}%)", "mech",
                 "信用未额外走坏=更像仓位去杠杆而非基本面")

    # 4) 黄金 GLD
    if not np.isnan(gld_r):
        if gld_r <= -0.005:
            tell("黄金 GLD", f"{gld_r*100:+.1f}%", "mech", "黄金跟跌=典型流动性挤兑(1987/2020-3 初期)=机械")
        elif gld_r >= 0.005:
            tell("黄金 GLD", f"{gld_r*100:+.1f}%", "reco", "黄金上涨=有序避险")
        else:
            tell("黄金 GLD", f"{gld_r*100:+.1f}%", "neutral", "黄金基本走平")

    # 5) 防御 vs 周期
    if not np.isnan(defensive_r) and not np.isnan(xlk_r):
        spread = defensive_r - xlk_r
        if spread > 0.02:
            tell("防御 vs 科技(XLP/XLU−XLK)", f"{spread*100:+.1f}%", "reco",
                 "资金往防御板块轮动、科技独差=对经济定价=衰退型")
        else:
            tell("防御 vs 科技(XLP/XLU−XLK)", f"{spread*100:+.1f}%", "mech",
                 "各板块无差别同跌=去杠杆而非轮动=机械型")

    # 6) 衰退闸门语境
    if gate_votes is not None:
        if gate_votes >= 2:
            tell("衰退闸门票数", f"{gate_votes}/6", "reco", "宏观已亮红=崩在衰退语境里=衰退型")
        elif gate_votes == 0:
            tell("衰退闸门票数", f"{gate_votes}/6", "mech", "宏观全绿里突然崩=正是 1987-type 特征=机械型")
        else:
            tell("衰退闸门票数", f"{gate_votes}/6", "neutral", "宏观有零星红灯,语境模糊")
